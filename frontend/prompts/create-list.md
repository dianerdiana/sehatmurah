...
Saya butuh rencana komprehensif beserta kode implementasi lengkap untuk membuat halaman "Daftar Spesialis" (Specialists List). Silakan gunakan struktur halaman `frontend/src/routes/_layout-dashboard/app.appointments.tsx` sebagai referensi acuan standar pengkodean Anda.

### 1. Konteks & Struktur File

- **Target Halaman**: `frontend/src/routes/_layout-dashboard/app.specialists.tsx`
- **Folder Komponen Kustom**: `frontend/src/modules/specialists/components/`
- **Sumber Data Fetching**: `frontend/src/modules/specialists/specialist.query.ts`
...
- **Operasi Server-Side**: Fitur-fitur berikut WAJIB menggunakan fetching sisi server melalui TanStack Query (JANGAN menggunakan filtering/sorting/pagination di sisi klien):
  - **Pencarian (Search)**: Input untuk mencari spesialis (menggunakan debounce jika memungkinkan).
...
- **Kolom Aksi (Actions Column)**: Tambahkan kolom `Actions` di akhir baris tabel yang berisi 3 tombol ikon:
  1. **Ikon Edit**: Mengarahkan pengguna ke halaman detail edit di `/app/specialists/$specialistId/edit`.
  2. **Ikon Eye (Lihat)**: Menampilkan Shadcn `Dialog` (Modal) yang berisi detail lengkap data spesialis tersebut.
...
1. File rute utama (`app.specialists.tsx`).
2. File definisi kolom (`columns.tsx`) yang mencakup implementasi fitur `Actions` (Edit link, Detail Modal, & Delete Popover).
3. Komponen wrapper untuk filter/pencarian kustom di dalam folder modul spesialis.
