...
export function PublicFacingLayout({
  children,
}: {
        ...
        <div className="mx-auto max-w-160">
          <div className="flex items-center justify-between rounded-t-2xl border-t border-gray-50 bg-white px-4 py-6">
            {navigation.map((nav) => (
              <Link
                to={nav.href}
                className="grid w-[112.33px] place-items-center gap-1.5"
              >
                {({ isActive }) => (
                  <>
                    <img
                    ...
                    />
                    <p
                      className={`font-bold leading-[20.16px] ${isActive ? 'text-primary' : 'text-gray-500'}`}
                    >
                      {nav.title}
                    </p>
                  </>
